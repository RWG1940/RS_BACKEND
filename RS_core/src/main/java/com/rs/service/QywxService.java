...
public class QywxService {
    ...
    public void syncQywxData() {
        ...
        System.out.println("已获取到部门信息" + deptList);
        ...
        for (QywxDepartment dept : deptEntities) {
            ...
            for (Map<String, Object> u : userList) {
                ...
                // 检查是否已经存在该用户
                QywxUser existingUser = userMapper.selectOne(new QueryWrapper<QywxUser>().eq("userid", userId));
                if (existingUser == null) {
                    ...
                    // 如果用户已存在，可以选择更新
                    existingUser.setName((String) u.get("name"));
                    existingUser.setDepartmentId(Long.valueOf(((List<Integer>) u.get("department")).get(0)));
                    userMapper.updateById(existingUser);  // 更新已存在的用户
                }
            }
        }
    }
    ...
    /**
     * 获取 launch_code，用于唤起企微客户端聊天页面
     */
    public Map<String, Object> getLaunchCode(String operatorUserId, String targetUserId) {
        String accessToken = getAccessToken();
        String url = "https://qyapi.weixin.qq.com/cgi-bin/get_launch_code?access_token=" + accessToken;

        Map<String, Object> requestBody = new HashMap<>();
        requestBody.put("operator_userid", operatorUserId);

        Map<String, Object> singleChat = new HashMap<>();
        singleChat.put("userid", targetUserId);
        requestBody.put("single_chat", singleChat);
        System.out.println(requestBody.toString());
        Map<String, Object> response = restTemplate.postForObject(url, requestBody, Map.class);
        if (response != null && "0".equals(response.get("errcode").toString())) {
            return response;
        }
        throw new RuntimeException("获取 launch_code 失败: " + response);
    }
    /**
     *   获取个人信息
     */
