...
import com.rs.domain.dto.LaunchCodeRequest;
...
public class QywxController {
    ...
    @PostMapping("/getLaunchCode")
    @ApiOperation("获取企业微信联系人唤起码")
    public Map<String, Object> getLaunchCode(@RequestBody LaunchCodeRequest body) {
        String operatorUserId = body.getOperator_userid();
        String targetUserId = body.getSingle_chat().getUserid();
        return qywxService.getLaunchCode(operatorUserId, targetUserId);
    }

    @GetMapping("/getUserInfo")
    @ApiOperation("获取企业微信用户信息")
    public Map<String, Object> getUserInfo(@RequestParam String userid) {
        return qywxService.getUserInfo(userid);
    }
